    ...
    extends TestCase
{
        ...
        throws Exception
    {
        ...
        PrivateKey key = new JcaPEMKeyConverter().setProvider("SC").getPrivateKey(encInfo.decryptPrivateKeyInfo(new JceOpenSSLPKCS8DecryptorProviderBuilder().setProvider("SC").build("hello".toCharArray())));

        encInfo = new PKCS8EncryptedPrivateKeyInfo(desEDEVec);

        PrivateKey rdKey = new JcaPEMKeyConverter().setProvider("SC").getPrivateKey(encInfo.decryptPrivateKeyInfo(new JceOpenSSLPKCS8DecryptorProviderBuilder().setProvider("SC").build("hello".toCharArray())));

        TestCase.assertEquals(key, rdKey);

        encInfo = new PKCS8EncryptedPrivateKeyInfo(desEDEpkcs12Vec);

        rdKey = new JcaPEMKeyConverter().setProvider("SC").getPrivateKey(encInfo.decryptPrivateKeyInfo(new JceOpenSSLPKCS8DecryptorProviderBuilder().setProvider("SC").build("hello".toCharArray())));
